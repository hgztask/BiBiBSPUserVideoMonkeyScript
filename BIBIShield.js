...
// @require      https://hangexi.gitee.io/bibibspuservideomonkeyscript/Util.js
...
function getChannelVideoRules(element) {//获取频道界面单个的视频信息
...
function getVideoCommentAreaOrTrendsLandlord(v) {//获取动态页面-评论区信息-单个元素信息-楼主
    return {
...
function getVideoCommentAreaOrTrendsStorey(j) {//获取动态页面-评论区信息-单个元素信息-楼层
    return {
    ...
    for (let entry of entries) {
        ...
        if (url.includes("api.bilibili.com/x/v2/reply/main?csrf=") || url.includes("api.bilibili.com/x/v2/reply/reply?csrf=")) {
            ...
            for (let v of list) {//针对于评论区
                ...
                const data = getVideoCommentAreaOrTrendsLandlord(usercontentWarp);
...
function searchColumn() {//根据规则屏蔽搜索专栏项目
    const interval = setInterval(() => {
...
function hideDisplayHomeLaylout() {//隐藏显示面板
...
function homePrefecture() {//针对于分区的广告页脚信息屏蔽
