...
style="color: ${color}; ">${Util.toTimeString()}${content}屏蔽用户【${name}】uid=<a href="https://space.bilibili.com/${uid}" target="_blank">【${uid}】</a>标题【<a href="${videoHref}" target="_blank">${title}</a>】</b>
    ...
    printCommentOn(color, content, name, uid, primaryContent) {
        Util.printElement("#outputInfo", `
<dd>
<bstyle="color: ${color}; ">${Util.toTimeString()}${content} 屏蔽用户【${name}】uid=<a href="https://space.bilibili.com/${uid}" target="_blank">【${uid}】</a>
...
<button @click="okVideoSelectBut">确定</button>
 ...
 <ol>
 <li>
 <pre style="white-space: pre-wrap">脚本中会对要匹配的内容进行去除空格和转成小写，比如有个内容是【不 要笑 挑战ChallEnGE】，会被识别称为【不要笑挑战challenge】</pre>
 </li>
...
<optionv-for="item in inoutRUleModelList" :value="item">{{item}}</option>
</select>
...
<div><select v-model="requestTypeSelect"><optionv-for="item in requestType" :value="item">{{item}}</option></select></div>
...
<select v-model="mainPartitionSelect"><optionv-for="(item,key) in partitionObjList" :value="key">{{key}}</option></select>
        ...
        delRightChatLayout() {
            ...
            if (liveData.isSystemRedTip) {
                const interval = setInterval(() => {//移除右侧的聊天布局系统提示
                    const tempE = document.getElementsByClassName("chat-itemconvention-msg border-box")[0];
                    if (tempE) {
                        tempE.remove();
                        clearInterval(interval);
