...
import videoPlayPageCommon from "./video_playCommon.ts";
import urlUtil from "../core/urlUtil.ts";

//判断是否是视频播放页
const isVideoPlayPage = (url: any = window.location.href) => {
...
const getGetTheVideoListOnTheRight = async () => {
    ...
    if (nextPlayEl) {
        elList.push(nextPlayEl as HTMLElement)
    }
    const list: any[] = [];
    for (let el of elList) {
        try {
            ...
            if (!elInfo) continue;
            const titleEl = elInfo.querySelector(".title");
            const title = titleEl?.title || '';
            const nameEl = elInfo.querySelector(".upname .name");
            ...
            let nPlayCount: any = playInfo.match(/<\/svg>(.*)<svg/s)?.[1].trim()
            nPlayCount = strFormatUtil.toPlayCountOrBulletChat(nPlayCount)
            let nBulletChat: any = playInfo.match(/class="dm".+<\/svg>(.+)$/s)?.[1].trim()
            nBulletChat = strFormatUtil.toPlayCountOrBulletChat(nBulletChat)
            let nDuration: any = el.querySelector('.duration')?.textContent.trim()
...
const getPlayerVideoList = async () => {
    ...
    const data: { list: { title: string | undefined, el: Element }[], cancelEl: Element | null } = {
        list: [],
        cancelEl: null
    }
