...
const getPlayCountAndBulletChatAndDuration = (el: any) => {
    ...
    let nBulletChat: any = playInfo.match(/class="dm-icon".+<\/svg>(.+)$/s)?.[1].trim()
...
const getRightVideoDataList = (elList: any) => {
    const list = []
...
const insertTagShieldButton = async () => {
    ...
    butEl.addEventListener('click', () => {
        ...
        eventEmitter.send('sheet-dialog', {
            ...
            list, title: '选择标签',
            optionsClick({value}: { value: string }) {
