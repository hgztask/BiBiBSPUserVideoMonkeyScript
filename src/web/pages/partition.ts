 ...
 * @param url
 */
const isNewPartition = (url: any = window.location.href) => {
...
const getVVideoDataList = async () => {
    ...
    for (let el of elList) {
        ...
        let nPlayCount: any = playAndDmu[0]?.textContent.trim()
        nPlayCount = strFormatUtil.toPlayCountOrBulletChat(nPlayCount)
        let nBulletChat: any = playAndDmu[1]?.textContent.trim()
