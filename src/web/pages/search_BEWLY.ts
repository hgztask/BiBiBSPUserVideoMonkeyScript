import BEWLYCommon from "./home_BEWLYCommon.ts";
...
const getVideoList = async () => {
    ...
    for (const el of elList) {
        ...
        if (!titleEl || !nameEl || !viewEl || !durationEl) {
            continue;
        }
        ...
        const insertionPositionEl = nameEl.parentElement ?? el;
        list.push({
...
const userListInstallAddButton = async () => {
    ...
    for (let el of gridEls) {
        ...
        const userUrl = (el as HTMLAnchorElement).href ?? '';
        if (!nameEl) continue;
        const name = nameEl.textContent?.trim() ?? '';
        const uid = urlUtil.getUrlUID(userUrl);
        const insertionPositionEl = el.querySelector('[flex*="col"]>[items-center]:last-child') as HTMLElement | null;
        shielding.addBlockButton({
            data: {
                ...
                insertionPositionEl: insertionPositionEl ?? (el as HTMLElement),
                explicitSubjectEl: el as HTMLElement
            }
        })
    }
}
...
const searchLiveListIntervalExecutor = new IntervalExecutor(async () => {
    ...
    for (let el of be_wly_el.querySelectorAll(".grid-adaptive>.video-card-container")) {
        ...
        list.push({
            ...
            el: el as HTMLElement,
            partition,
            explicitSubjectEl: explicitSubjectEl as HTMLElement | undefined,
            insertionPositionEl: insertionPositionEl ?? (el as HTMLElement)
        })
    }
    for (let v of list) {
...
export default {
    isUrlPage(url: string, title: string) {
