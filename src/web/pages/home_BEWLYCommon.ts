...
const getRankingLeftTabs = async () => {
    ...
    const elList = await elUtil.findElements('ul[flex="~ col gap-2"]>li', {doc: beEl ?? undefined})
    const list: any[] = [];
...
const getRightTabs = async () => {
    const beEl = await getBewlyEl();
    const els = await elUtil.findElements(".dock-content-inner>.b-tooltip-wrapper", {doc: beEl ?? undefined})
    const list: any[] = [];
    for (let el of els) {
...
const getVideoList = async () => {
    ...
    const elList = await elUtil.findElements(homeVideoCardListSelector, {doc: be_wly_el ?? undefined})
    const list: any[] = [];
    const isBEWLYCatPluginVal = await isBEWLYCatPlugin()
    for (let el of elList) {
        const titleEl = el.querySelector('.video-card-title>a');
        ...
        if (isBEWLYCatPluginVal) {
            ...
            nPlayCount = othersStatsEls?.[0]?.textContent.trim() ?? null;
            bulletChat = othersStatsEls?.[1]?.textContent.trim() ?? null;
            ...
            if ((playInfoEl?.querySelectorAll('span').length ?? 0) < 2) {
                bulletChat = -1
            } else {
...
const getHistoryVideoDataList = async () => {
    const beEL = await getBewlyEl()
    const elList = await elUtil.findElements("a.group[flex][cursor-pointer]", {doc: beEL ?? undefined})
    const list = []
    for (let el of elList) {
        ...
        const bv = urlUtil.getUrlBV(videoUrl)
        const userUrl = userEl instanceof HTMLAnchorElement ? userEl.href : ''
        const uid = urlUtil.getUrlUID(userUrl ?? '')
...
const searchBoxInsertListener = async () => {
    const beEl = await getBewlyEl()
    const input = await elUtil.findElement('[placeholder="搜索观看历史"]', {doc: beEl ?? undefined})
